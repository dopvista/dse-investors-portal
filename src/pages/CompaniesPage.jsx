            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              <thead>
                <tr style={{ background: C.gray50 }}>
                  {["#", "Company Name", "Current Price (TZS)", "Change", "Previous Price (TZS)", "Last Price Update", ""].map(h => (
                    <th key={h} style={{ padding: "10px 18px", textAlign: ["Current Price (TZS)", "Change", "Previous Price (TZS)"].includes(h) ? "right" : "left", color: C.gray400, fontWeight: 700, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.06em", borderBottom: `1px solid ${C.gray200}`, whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((c, i) => {
                  ...
                  return (
                    <tr key={c.id} style={{ borderBottom: `1px solid ${C.gray100}`, transition: "background 0.15s" }}
                      ...
                      <td style={{ padding: "14px 18px", textAlign: "right" }}>
                        <ActionMenu actions={[
                          { icon: "💰", label: "Update Price", onClick: () => setUpdateModal({ open: true, company: c }) },
                          { icon: "📈", label: updating === c.id || loadingHistory === c.id ? "Loading..." : "Price History", onClick: () => viewHistory(c) },
