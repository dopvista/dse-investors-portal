...
function ConfirmActionModal({ action, count = 1, company, onConfirm, onClose }) {
  ...
  const subtitle    = count > 1 ? `${count} transactions selected` : company ? `${company}` : "1 transaction selected";
...
export default function TransactionsPage({ companies, transactions, setTransactions, showToast, role, cdsNumber }) {
  ...
  // Default status per role — VR sees confirmed first, RO sees verified first
  const defaultStatus = isVR ? "confirmed" : isRO ? "verified" : "All";
  ...
  // ── Stats (always from full unfiltered CDS-scoped list) ───────────
  const stats = useMemo(() => {
    const mine = isSAAD
      ? (cdsNumber ? transactions.filter(t => t.cds_number === cdsNumber) : transactions)
      : transactions.filter(t => t.cds_number === cdsNumber);
    ...
    return {
      ...
      confirmed:    mine.filter(t => t.status === "confirmed").length,
      verified:     mine.filter(t => t.status === "verified").length,
      rejected:     mine.filter(t => t.status === "rejected").length,
    };
  }, [transactions, cdsNumber, isSAAD]);

  // ── Filter ────────────────────────────────────────────────────────
  const filtered = useMemo(() => {
    let list = transactions;

    // 1. CDS visibility — non-admins always see only their CDS
    if (!isSAAD) {
    ...
    // 3. Status filter — applies to all roles; dropdown controls this
    ...
    return list;
  }, [transactions, search, typeFilter, statusFilter, cdsNumber, isSAAD]);
  ...
  const selectedConfirmed = [...selected].filter(id => transactions.find(t => t.id === id)?.status === "confirmed");
  ...
  // ── Status dropdown options per role ──────────────────────────────
  const statusOptions = isRO
    ? [["All","All Statuses"],["verified","✔️ Verified"]]
    : isVR
    ? [["All","All Statuses"],["confirmed","✅ Confirmed"],["verified","✔️ Verified"],["rejected","✖ Rejected"]]
    : [["All","All Statuses"],["pending","🕐 Pending"],["confirmed","✅ Confirmed"],["verified","✔️ Verified"],["rejected","✖ Rejected"]];
    ...
    <div style={{ height: "calc(100vh - 118px)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <SectionCard
          ...
          subtitle={
            statusFilter !== "All"
              ? `Filtered: ${statusOptions.find(o => o[0] === statusFilter)?.[1] || statusFilter} transactions`
              : isVR   ? "Transactions for your review"
          ...
          {stats.total === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 20px", color: C.gray400 }}>
              ...
              <div style={{ fontSize: 13 }}>
                {isDE ? "Click \"Record Transaction\" to add your first buy or sell" : "Transactions will appear here once created"}
              </div>
            </div>
          ) : filtered.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 20px", color: C.gray400 }}>
              ...
              <button onClick={() => { setSearch(""); setTypeFilter("All"); setStatusFilter(defaultStatus); }}
