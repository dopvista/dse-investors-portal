  ...
  return (
    <div style={{ minHeight: "100vh", width: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: C.navy, fontFamily: "'Inter', sans-serif", padding: 16, boxSizing: "border-box" }}>
      ...
      <div style={{
        ...
        height: "min(520px, 85vh)",      /* ← fixed height instead of aspect ratio */
        background: "white",
        borderRadius: 28,
        ...
        {/* ── LEFT: Photo slider ── */}
        <div style={{ position: "relative", background: ADVERTS[activeAd].color, transition: "background 1s ease", overflow: "hidden", display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 36px" }}>
          {ADVERTS.map((ad, i) => (
          ...
          <div style={{ position: "relative", zIndex: 2 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: C.gold, letterSpacing: 1, marginBottom: 14, textTransform: "uppercase" }}>DAR ES SALAAM STOCK EXCHANGE</div>
            {ADVERTS.map((ad, i) => (
              <div key={ad.id} style={{ display: i === activeAd ? "block" : "none", animation: "fadeIn 0.8s ease-out" }}>
                <h2 style={{ fontSize: "clamp(24px, 3.5vw, 34px)", fontWeight: 800, color: "white", margin: "0 0 6px 0", lineHeight: 1.2, textShadow: "0 2px 8px rgba(0,0,0,0.2)" }}>{ad.title}</h2>
        ...
        {/* ── RIGHT: Form ── */}
        <div style={{ background: "white", display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 28px" }}>
          <div style={{ width: "100%" }}>

            {/* Header */}
            <div style={{ textAlign: "center", marginBottom: 20 }}>
              <img src={logo} alt="DSE" style={{ width: 48, height: 48, borderRadius: 12, objectFit: "cover", marginBottom: 10, boxShadow: "0 4px 16px rgba(0,0,0,0.2)" }} />
              ...
              {view === "reset" && (
                <div style={{ marginTop: 10, background: `${C.gold}18`, border: `1px solid ${C.gold}55`, borderRadius: 8, padding: "7px 10px", fontSize: 11, color: C.gold, fontWeight: 600 }}>
            ...
            {/* Banners */}
            {error   && <div style={{ background: "#fef2f2", border: "1px solid #fecaca", color: "#dc2626", borderRadius: 10, padding: "9px 12px", fontSize: 12, marginBottom: 14 }}>{error}</div>}
            {success && <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", color: "#16a34a", borderRadius: 10, padding: "9px 12px", fontSize: 12, marginBottom: 14 }}>{success}</div>}
            ...
            {view === "reset" && (
              <form onSubmit={handleReset}>
                ...
                <button type="button" onClick={() => { setView("login"); setError(""); setSuccess(""); }}
                  style={{ width: "100%", padding: "9px", borderRadius: 10, border: `1.5px solid ${C.gray200}`, background: C.white, color: C.gray400, fontWeight: 600, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}
            ...
            {/* Footer */}
            <div style={{ marginTop: 20, paddingTop: 16, borderTop: `1px solid ${C.gray200}`, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
